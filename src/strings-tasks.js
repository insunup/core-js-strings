...
function formatTime(minutes, seconds) {
  // Ensure non-negative integers for minutes and seconds
  ...
  const formattedSeconds = String(validSeconds).padStart(2, '0');
...
function countVowels(str) {
  // Using a regular expression to match vowels and counting the matches
  const vowelMatches = str.match(/[aeiouyAEIOUY]/g);

  // Return the number of vowel matches or 0 if there are none
...
function findLongestWord(sentence) {
  ...
  // Используем метод reduce для поиска самого длинного слова
  const result = words.reduce(
    (acc, word) => {
      const cleanedWord = word.replace(/[.,/#!$%^&*;:{}=\-_`~()]/g, '');

      if (cleanedWord.length > acc.maxLength) {
        return { longestWord: cleanedWord, maxLength: cleanedWord.length };
      }
...
function reverseWords(str) {
  ...
  // Собираем обратно строку из перевернутых слов
...
function invertCase(str) {
  ...
  const invertedStr = str.split('').map(invertChar).join('');
...
function extractNameFromTemplate(value) {
  ...
  // Если найдено совпадение, возвращаем извлеченное имя, иначе возвращаем пустую строку
